...
class Logger:
    ...
    def init_log(self):
        num_time_steps = int(np.ceil(self.cfg.max_time * self.cfg.log_fps)) + 1
        print("dataset will contain", num_time_steps, "timesteps!")
        for field in self.fields:
            print("Creating", field, "dataset in log!")
            if field == 'position':
                self.file.create_dataset(field, (3 * self.cfg.num_particles, num_time_steps), dtype='f')
            if field == 'velocity':
                self.file.create_dataset(field, (3 * self.cfg.num_particles, num_time_steps), dtype='f')
            if field == 'acceleration':
                self.file.create_dataset(field, (3 * self.cfg.num_particles, num_time_steps), dtype='f')
            if field == 'density':
                self.file.create_dataset(field, (1 * self.cfg.num_particles, num_time_steps), dtype='f')
            if field == 'pressure':
                self.file.create_dataset(field, (1 * self.cfg.num_particles, num_time_steps), dtype='f')
